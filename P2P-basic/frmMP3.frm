...
Private Sub cmdOpen_Click()
    With MediaPlayer1
        If Not FileOpen Then
            ...
            .AutoStart = False
            cmdOpen.Caption = "■"
            cmdOpen.ToolTipText = "연주중인 MP3 닫기"
            Timer6.Enabled = True
        Else
            .Stop
            cmdOpen.Caption = "▲"
            FileOpen = False
            cmdOpen.ToolTipText = "선택된 MP3 열기"
...
Private Sub cmdSaveInfo_Click()
    If FileOpen Then
        MsgBox "파일이 열려있으므로 저장을 할 수 없습니다.", vbCritical, "MP3 태그 저장 에러"
        Exit Sub
    Else:
        If txtTitle.Text = "" Then
            MsgBox "먼저 MP3 파일을 선택하거나 제목을 입력하세요.", vbCritical, "MP3 태그 저장 에러"
            Exit Sub
        End If
    End If
...
ErrHandle:
    If Err.Number = 75 Then
        MsgBox "MP3 파일이 사용 중이거나 쓰기금지 상태입니다."
    Else
...
Private Sub MediaPlayer1_OpenStateChange(ByVal OldState As Long, ByVal NewState As Long)
    ...
    lblTotalTime = "전체시간 : " & Format(min, "0#") & ":" & Format(sec, "0#") 'format time to 00:00
